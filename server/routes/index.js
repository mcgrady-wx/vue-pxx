...
router.get('/recommend/api', function (req, res, next) {
    ...
    // 3. 批量插入数据库表
    // 3.1 数据库查询的语句
    let sqlStr = "INSERT INTO pxx_recommend(`goods_id`,`goods_name`,`short_name`, `thumb_url`, `hd_thumb_url`, `image_url`, `price`, `normal_price`, `market_price`, `sales_tip`, `hd_url`) VALUES ?";
    // 3.2 执行语句
...
router.get('/api/homecasual', (req, res) => {
    ...
    // 1.1 数据库查询的语句
    let sqlStr = 'SELECT * FROM pxx_homecasual';
    // 1.2 执行语句
...
router.get('/api/recommendshoplist', (req, res) => {
    ...
    // 1.1 数据库查询的语句
    let sqlStr = 'SELECT * FROM pxx_recommend LIMIT ' + (pageNo - 1) * pageSize + ',' + pageSize;
    // console.log(sqlStr);
...
router.post('/api/login_code', (req, res) => {
    ...
    let sqlStr = "SELECT * FROM pxx_user_info WHERE user_phone = '" + phone + "' LIMIT 1";
    ...
    conn.query(sqlStr, (error, results, fields) => {
        if (error) {
            ...
            if (results[0]) {  // 用户已经存在
                ...
                const addSql = "INSERT INTO pxx_user_info(user_name, user_phone) VALUES (?, ?)";
...
router.post('/api/login_pwd', (req, res) => {
    ...
    // 3. 查询数据
    let sqlStr = "SELECT * FROM pxx_user_info WHERE user_name = '" + user_name + "' LIMIT 1";
    conn.query(sqlStr, (error, results, fields) => {
        if (error) {
            ...
            if (results[0]) {  // 用户已经存在
                ...
                const addSql = "INSERT INTO pxx_user_info(user_name, user_pwd) VALUES (?, ?)";
...
router.get('/api/user_info', (req, res) => {
	...
	console.log(userId)
    // 1.1 数据库查询的语句
    let sqlStr = "SELECT * FROM pxx_user_info WHERE id = '" + userId + "' LIMIT 1";
    conn.query(sqlStr, (error, results, fields) => {
        if (error) {
            res.json({err_code: 0, message: '请求数据失败'});
        } else {
...
router.post('/api/change_user_msg', (req, res) => {
    ...
    // 3. 更新数据
    let sqlStr = "UPDATE pxx_user_info SET user_name = ? , user_sex = ?, user_address = ?, user_birthday = ?, user_sign = ? WHERE id = " + id;
...
router.post('/api/add_shop_cart', (req, res) => {
    ...
    // 3. 查询数据
    let sql_str = "SELECT * FROM pxx_cart WHERE goods_id = '" + goods_id + "' LIMIT 1";
    conn.query(sql_str, (error, results, fields) => {
        if (error) {
            ...
            if (results[0]) { // 3.1 商品已经存在
                ...
                let sql_str = "UPDATE pxx_cart SET buy_count = " + buy_count + " WHERE goods_id = '" + goods_id + "'";
                conn.query(sql_str, (error, results, fields) => {
                    if (error) {
                ...
                let add_sql = "INSERT INTO pxx_cart(goods_id, goods_name, thumb_url, price, buy_count, is_pay) VALUES (?, ?, ?, ?, ?, ?)";
...
router.get('/api/cart_goods', (req, res) => {
    ...
    // 1.1 数据库查询的语句
    let sqlStr = "SELECT * FROM pxx_cart";
    conn.query(sqlStr, (error, results, fields) => {
        if (error) {
            res.json({err_code: 0, message: '请求数据失败'});
        } else {
